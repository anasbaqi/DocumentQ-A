...
def initiate_llm_HF(input_type, hf_token):
    #etheir use OpenAI LLM or huggingface LLM:
    llm = HuggingFaceHub(repo_id="google/flan-t5-large", model_kwargs={"temperature": 1, "max_length": 512}, verbose=True, huggingfacehub_api_token=hf_token)

    # Get your embeddings engine ready
    embeddings = HuggingFaceEmbeddings(huggingfacehub_api_token=hf_token)
...
def initiate_llm(input_type, openai_key):
...
def main():
    ...
    st.write("By [Anas Abdulbaqi](https://www.linkedin.com/in/anas-abdulbaqi/)")
    ...
    if option in ('Excel File', 'Text File', 'PDF File'):
        ...
        if user_choice is not None:
            ...
            if model_type in ('OpenAI', 'HuggingFace'):
                ...
                api_key = st.text_input("Enter your API key to get started", type="password")
                if api_key:
                    if model_type == 'OpenAI':
                        qa = initiate_llm(input_type, api_key)
                    else:
                        qa = initiate_llm_HF(input_type, api_key)
